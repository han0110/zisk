...
pub fn miller_loop_batch_bls12_381(
    ...
    // Before the loop starts, compute xp' = (-xp/yp)·1/(1+u) and yp' = (1/yp)·1/(1+u)
    let n = g1_points.len();
    let mut xp_primes = [[0u64; 12]; super::PAIRING_BATCH_BLS12];
    let mut yp_primes = [[0u64; 12]; super::PAIRING_BATCH_BLS12];
    ...
    let mut r = [[0u64; 24]; super::PAIRING_BATCH_BLS12];
    (0..n).for_each(|i| r[i] = g2_points[i][0..24].try_into().unwrap());
    let mut f = [0u64; 72];
    f[0] = 1;
    for &bit in X_ABS_BIN_BE.iter().skip(1) {
        // Compute f = f² · line_{twist(r),twist(r)}(p)
