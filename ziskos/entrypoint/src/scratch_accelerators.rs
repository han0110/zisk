...
/// Scratch-arena replacement for the std `vec!` macro. Brought into scope in the
/// which shadows the prelude `vec!`. On host builds the helpers produce a std
/// `Vec`, so this is also correct off-target.
#[macro_export]
macro_rules! scratch_vec {
    () => {
        $crate::scratch_accelerators::new_scratch_vec(0)
    };
    ($elem:expr; $n:expr) => {
        $crate::scratch_accelerators::new_scratch_vec_filled_z($n, $elem)
    };
    ($($x:expr),+ $(,)?) => {
        $crate::scratch_accelerators::scratch_vec_from_slice(&[$($x),+])
    };
}
pub(crate) use scratch_vec;
