...
pub struct Coordinator {
    ...
    /// Maps JobId -> Vec of mpsc::UnboundedSender for streaming updates.
    client_subscriptions: Arc<DashMap<JobId, Vec<mpsc::UnboundedSender<ProofStatusUpdate>>>>,
}
...
impl Coordinator {
    ...
    /// * `sender` - Channel to send completion events
    ///
    /// # Returns
    ///
    ...
    pub async fn subscribe_to_proof_completion(
        &self,
        job_id: &JobId,
        sender: mpsc::UnboundedSender<ProofStatusUpdate>,
        ...
        if let Some((_, senders)) = self.client_subscriptions.remove(job_id) {
            ...
            for sender in senders {
                match sender.send(update.clone()) {
                    Ok(_) => sent += 1,
                    Err(_) => failed += 1, // Client disconnected
                }
            }
